  ...
  return (
    <div className={cn("flex flex-wrap items-end gap-x-3 gap-y-3", compact && "flex-col items-stretch gap-2")}>
      <div className={cn("flex-1 min-w-[200px] max-w-[320px] space-y-1.5", compact && "min-w-0 max-w-none space-y-1")}>
        <Label htmlFor="community-search" className="text-xs leading-none">
          {t("tmpl_comm_search")}
        </Label>
      ...
      <div className={cn("flex items-end gap-2", compact && "grid grid-cols-2 gap-2")}>
        <div className={cn("w-[160px] shrink-0 space-y-1.5", compact && "w-full min-w-0 space-y-1")}>
          <Label htmlFor="community-type" className="text-xs leading-none">
            {t("tmpl_comm_file_type")}
          </Label>
          <Select value={selectedFileType} onValueChange={(v) => onFileTypeChange(v as CommunityFileType)}>
            <SelectTrigger id="community-type" className={cn("h-9", compact && "h-8")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className={cn("w-[160px] shrink-0 space-y-1.5", compact && "w-full min-w-0 space-y-1")}>
          <Label htmlFor="community-sort" className="text-xs leading-none">
            {t("tmpl_comm_sort")}
          </Label>
          <Select value={selectedSort} onValueChange={(v) => onSortChange(v as CommunitySort)}>
            <SelectTrigger id="community-sort" className={cn("h-9", compact && "h-8")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
