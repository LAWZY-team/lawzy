...
import { AddNewTRModal } from "@/components/lpms/tabular/AddNewTRModal";
...
import { PeopleModal } from "@/components/lpms/shared/PeopleModal";
...
function activeSectionFromSegments(
    ...
    if (segments[0] === "tabular-analysis") return "reviews";
    return "documents";
}
...
function shouldShowWorkspaceShell(segments: string[]) {
    ...
    return segments[0] === "assistant" || segments[0] === "tabular-analysis";
}
...
export function ProjectWorkspaceProvider({
    ...
    const createChat = useCallback(async () => {
        ...
        try {
            ...
            if (id) {
                ...
                router.push(`/lpms/matters/${projectId}/assistant/chat/${id}`);
            }
        } finally {
    ...
    async function handleCreateReview(
        ...
        try {
            ...
            router.push(`/lpms/matters/${projectId}/tabular-analysis/${review.id}`);
        } finally {
    ...
    async function confirmProjectDelete() {
        ...
        try {
            ...
            window.setTimeout(() => router.push("/lpms/matters"), 500);
        } catch (error) {
        ...
        <ProjectWorkspaceContext.Provider value={value}>
            <div className="relative flex h-full min-h-0 flex-1 flex-col overflow-hidden">
                <ProjectPageHeader
                    ...
                    onBackToProjects={() => router.push("/lpms/matters")}
    ...
    return (
        <TableToolbar
            ...
            onChange={(next) => {
                const href =
                    next === "documents"
                        ? `/lpms/matters/${projectId}`
                        : next === "assistant"
                          ? `/lpms/matters/${projectId}/assistant`
