...
function FeatureCard({ title, description, videoSrc, videoAlt, reverse = false }: {
  ...
  return (
    <div className={`group flex flex-col ${reverse ? "lg:flex-row-reverse" : "lg:flex-row"} items-center gap-8 lg:gap-1 bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 p-6 sm:p-8 md:p-10 lg:p-12 border border-gray-100`}>
      <div className="w-full h-full lg:w-3/5 flex-shrink-0">
        <div className="relative w-full sm:aspect-video lg:aspect-square lg:w-[600px] lg:h-[600px] rounded-2xl overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 shadow-md group-hover:shadow-lg transition-shadow duration-300">
      ...
      <div className="w-full lg:w-2/5 lg:px-4 lg:text-left flex flex-col justify-center text-center">
        <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-4 lg:mb-6 leading-tight">{title}</h3>
        <p className="text-base sm:text-lg lg:text-xl text-gray-600 leading-tight">{description}</p>
      </div>
    </div>
  );
}
      ...
      <section id="features" className="py-16 sm:py-20 md:py-28 lg:py-32">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          <div className="text-center mb-12 sm:mb-16 md:mb-20">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-4 sm:mb-6 tracking-tight">{t("features_title")}</h2>
            <p className="text-lg sm:text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">{t("features_subtitle")}</p>
          </div>
