      ...
      >
        <div className="container mx-auto px-4 py-1 flex justify-between items-center min-h-[60px] md:min-h-[72px] relative">
          ...
          <div className="hidden md:flex absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 space-x-8 items-center">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} className="text-gray-600 hover:text-black hover:font-semibold transition-all font-medium text-sm lg:text-base">
                {link.label}
              </a>
            ))}
          </div>
              ...
              <AnimatePresence>
                {isScrolled && (
                  <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: "auto", opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="overflow-hidden">
                    <div className="pr-2">
                      <Button type="button" className="bg-black hover:bg-gray-800 text-white h-9 rounded-full shadow-md whitespace-nowrap" onClick={onOpenSurvey}>
                        {t("try_free")}
                      </Button>
                    </div>
                  </motion.div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed inset-0 top-[80px] z-40 bg-white border-t border-gray-100 md:hidden flex flex-col p-6 space-y-6 shadow-xl">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} className="text-2xl font-semibold text-gray-800 hover:text-orange-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                {link.label}
              </a>
            ))}
            <div className="pt-6 border-t border-gray-100 w-full space-y-4">
              ...
              <Button className="w-full bg-black hover:bg-gray-800 text-white text-lg h-12 rounded-xl" onClick={() => { setIsMobileMenuOpen(false); onOpenSurvey?.(); }}>
                {t("try_free")}
              </Button>
            </div>
          </motion.div>
