  ...
  return (
    <section className="py-16">
      <div className="container mx-auto px-4">
        <div className="mb-16">
          <h3 className="text-2xl font-bold text-center text-gray-400 uppercase tracking-widest mb-10">{t("incubation_by")}</h3>
          <div className="flex flex-wrap justify-center gap-12 md:gap-24">
            {INCUBATED_BY.map((item, index) => (
              <div key={index} className="flex flex-col items-center max-w-[280px]">
                <div className={`h-24 w-auto relative mb-6 flex items-center justify-center ${item.extraClasses || ""}`}>
                  <Image src={item.logo} alt={item.alt} height={90} width={220} className="object-contain max-h-full w-auto" />
                </div>
                <div className="text-center">
                  <p className="font-medium text-gray-900 text-base mb-2">{item.org}</p>
