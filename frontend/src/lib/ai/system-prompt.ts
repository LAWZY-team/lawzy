...
6. TUYỆT ĐỐI KHÔNG SỬ DỤNG BẤT KỲ THẺ HTML NÀO (như <p>, <b>, <ul>, <i>, <br>...) trong bất kỳ trường nào của JSON. Chỉ sử dụng văn bản thuần túy (plain text), dùng ký tự xuống dòng \\n và markdown đơn giản như **in đậm** để nhấn mạnh.
...
Trong output JSON, khi trả về content/sections, hãy bao gồm đầy đủ phần header (quốc hiệu → số HĐ → căn cứ → lời mở đầu → thông tin bên A/B) trước các điều khoản. Dùng mergeFields cho mọi chỗ cần điền: số HĐ, ngày/tháng/năm/địa điểm ký, thông tin doanh nghiệp hai bên. Phần text không được chứa mã HTML mà chỉ là \\n, markdown **in đậm** và text thuần. Hệ thống frontend sẽ tự động phân tách và định dạng căn lề dựa vào vị trí của đoạn (các đoạn đầu tự động được căn giữa).
    ...
    "sections": [
      {
        ...
        "content": "Bên A đồng ý cung cấp cho Bên B dịch vụ với chi tiết như sau:\n- Yêu cầu 1: ...\n- Yêu cầu 2: ...\nGiá trị: {{SERVICE_DESCRIPTION}}",
