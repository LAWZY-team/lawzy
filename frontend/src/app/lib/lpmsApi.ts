/**
 * LPMS API client — all requests to the Node.js backend.
...
export async function createChat(payload?: {
    ...
    return apiRequest<{ id: string }>("/lpms/chats/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload ?? {}),
    });
...
export async function listChats(options?: { limit?: number }): Promise<Chat[]> {
    ...
    return apiRequest<Chat[]>(`/lpms/chats${query ? `?${query}` : ""}`);
}
...
export async function getChat(chatId: string): Promise<ChatDetailOut> {
    const raw = await apiRequest<ServerChatDetailOut>(`/lpms/chats/${chatId}`);
    const messages: Message[] = raw.messages.map((m) => {
        ...
        return {
            ...
            events: events ?? [{ type: "content", text: textContent }],
        };
    });
...
export async function renameChat(chatId: string, title: string): Promise<void> {
    await apiRequest(`/lpms/chats/${chatId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
...
export async function deleteChat(chatId: string): Promise<void> {
    await apiRequest(`/lpms/chats/${chatId}`, { method: "DELETE" });
}
...
export async function generateChatTitle(
    ...
    return apiRequest<{ title: string }>(`/lpms/chats/${chatId}/generate-title`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export async function streamChat(payload: {
    ...
    const authHeaders = await getAuthHeader();
    return fetch(`${API_BASE}/lpms/chat`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
