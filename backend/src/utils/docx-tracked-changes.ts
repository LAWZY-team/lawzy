...
/**
 * Walk document.xml and get the entry matching path
 */
function getZipEntryString(zip: JSZip, pathSlash: string): Promise<string | null> {
    const entry = getZipEntry(zip, pathSlash);
...
export async function extractDocxBodyText(bytes: Buffer): Promise<string> {
    ...
    const docXmlRaw = await getZipEntryString(zip, "word/document.xml");
    if (!docXmlRaw) return "";
...
export async function applyTrackedEdits(
    ...
    const author = opts?.author ?? "Lawzy";
    const now = new Date().toISOString();

    const zip = await JSZip.loadAsync(bytes);
    const docXmlRaw = await getZipEntryString(zip, "word/document.xml");
    if (!docXmlRaw) throw new Error("document.xml missing from docx");
    ...
    for (let editIdx = 0; editIdx < edits.length; editIdx++) {
        ...
        if (!selected) {
            errors.push({
                ...
                reason: sawAmbiguous
                    ...
                    : `Could not locate find="${truncate(find, 80)}" in the document. Re-read the document and copy context verbatim.`,
            });
            continue;
        }
        ...
        const changeId = `lawzy-${editIdx}-${Date.now()}`;
